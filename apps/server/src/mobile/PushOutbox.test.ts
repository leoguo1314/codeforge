...
const layer = it.layer(Layer.merge(sqliteLayer, outboxLayer));

layer("PushOutbox", (it) => {
  it.effect("persists, retries, and completes a canonical notification", () =>
    Effect.gen(function* () {
      yield* Migration0022;
      ...
      assert.equal(firstDue[0]?.deliveryId, deliveryId);
      assert.equal(firstDue[0]?.attemptCount, 0);
      assert.equal(firstDue[0]?.notification.title, notification.title);
